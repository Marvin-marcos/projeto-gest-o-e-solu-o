  ...
  <div class="container">
    <aside class="sidebar">
      <ul>
        ...
        <li onclick="window.location.href='../lucros/lucros.php'">Lucros</li>
...
function parseValorBR(valor) {
  ...
  // remove espaços, "R$" e separadores de milhar; troca vírgula por ponto
...
function getTabelaComoArray() {
  const tabela = document.getElementById("tabelaVendas");
  const headers = Array.from(tabela.rows[0].cells).map(th => th.innerText.trim());
  const linhas = [];
...
function salvarTabela() {
  ...
  // headers atuais
  const headers = Array.from(tabela.rows[0].cells).map(th => th.innerText.trim());
  const idxStatus = document.getElementById("statusHeader").cellIndex;

  // coluna de valor (tenta achar qualquer header que contenha "valor")
  const nomeColunaValor = headers.find(h => /valor/i.test(h)) || "Valor da Venda";

  // monta novosDados (envio ao PHP apenas do que é novo, como já fazia)
  const novosDados = [];
  for (let i = 1; i < tabela.rows.length; i++) {
    const row = tabela.rows[i];
    const linha = {};
    for (let j = 0; j < row.cells.length; j++) {
      const input = row.cells[j].querySelector("input, select");
      linha[headers[j]] = input ? input.value : "";
    }
    if (!dadosExistentes.includes(JSON.stringify(linha))) {
      novosDados.push(linha);
      dadosExistentes.push(JSON.stringify(linha));
    }
  }

  // --- snapshot COMPLETO para o localStorage (sempre) ---
  const { linhas: snapshot } = getTabelaComoArray();
  const total = snapshot.reduce((s, item) => s + parseValorBR(item[nomeColunaValor]), 0);

  localStorage.setItem("vendas", JSON.stringify(snapshot));
  localStorage.setItem("totalVendas", total.toFixed(2));
  console.log("Snapshot salvo no localStorage. Total:", total.toFixed(2));
  // ------------------------------------------------------

  // Posta ao PHP só se houver linhas novas
  if (novosDados.length === 0) {
    alert("Nenhum dado novo para salvar. (LocalStorage atualizado mesmo assim)");
    return;
  }
